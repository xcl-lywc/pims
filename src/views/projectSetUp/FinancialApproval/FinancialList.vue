...
<template>
  <div>
    ...
    <div class="block-wrap">
      <h4 class="block-title">财政意见列表</h4>
            ...
            <el-col :span="24">
              <el-table :data="tableData.rows" stripe style="width: 100%">
                ...
                <el-table-column prop="processVariables.cur_status" label="当前状态" width="">
                  <template slot-scope="scope">
                    <span v-if="scope.row.processVariables.cur_status == 11">待财政审批</span>
                  </template>  
                </el-table-column>
                <el-table-column label="操作" width="200" fixed="right" header-align="center" align="center">
                  <template slot-scope="scope">
                    ...
                    <el-button type="primary" icon="" size="small" @click.stop="financialClick(scope.row)">录入</el-button>
                  </template>
                </el-table-column>
              </el-table>
            </el-col>
  ...
  export default {
    ...
    methods: {
      ...
      */
       ...
       */
      financialClick(row){
        sessionStorage.setItem("rowFinancialInfo",JSON.stringify(row))
        // this.$router.push({path: '/project_set_up/financial/approval'});
        this.$router.push({
