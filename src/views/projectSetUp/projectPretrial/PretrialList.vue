...
<template>
  <div>
    ...
    <div class="block-wrap">
      <h4 class="block-title">项目预审列表</h4>
          ...
          <el-row>
            <el-col :span="24">
              <el-table :data="tableData.rows" stripe style="width: 100%">
                ...
                <el-table-column prop="processVariables.name" label="项目名称" width="300" show-overflow-tooltip>
                ...
                <el-table-column prop="processVariables.company" label="申报单位" show-overflow-tooltip>
                  <template slot-scope="scope">
                    <span>
                ...
                <el-table-column prop="processVariables.cur_status" label="当前状态" width="">
                  <template slot-scope="scope">
                    <span v-if="scope.row.processVariables.cur_status == 3">待确认预审专家</span>
                    <span v-if="scope.row.processVariables.cur_status == 4">待预审</span>
                  </template>
                </el-table-column>
                <el-table-column label="操作" width="280" header-align="center" align="center" fixed="right">
                  <template slot-scope="scope">
                    ...
                    <el-button type="primary" icon="" size="small" @click.stop="pretrialClick(scope.row)" 
                    v-if="scope.row.processVariables.cur_status==3 ? true:false">建立专家组
                      </el-button>
                    <el-button type="primary" icon="" size="small" @click.stop="pretrialClick(scope.row)" 
                    v-if="scope.row.processVariables.cur_status==4 ? true:false">预审
                      </el-button>
  ...
  export default {
    ...
    methods: {
      ...
      */
       ...
       */
      pretrialClick(row){
        sessionStorage.setItem("rowPretrialInfo",JSON.stringify(row))
        if(JSON.parse(sessionStorage.getItem("rowPretrialInfo")).processVariables.cur_status===4){
          this.$router.push({path: '/project_set_up/pretrial/expert'});
//           this.$router.push({
//             name:'PretrialExpert',
//             params:{
//               rotation:row.processVariables.budgetRotation,
//             }
//           })
        }else if(JSON.parse(sessionStorage.getItem("rowPretrialInfo")).processVariables.cur_status===3){
          this.$router.push({path: '/project_set_up/pretrial/expert_group'});
        }
