...
<template>
  <div>
  	...
  	<div class="block-wrap">
  		<h4 class="block-title">专家预审</h4>
  		<div class="block-body">
  			<div class="body-content body-contents">
					<el-row>
						<h4 class="block-depart-title expert-opinion">预审专家</h4>
						<div class="clear"></div>
						<div>
							<el-tag
		...
		<el-dialog
			...
			<el-form label-width="80px">
				...
				<el-form-item label="修改后是否再次邮件通知预审专家？" label-width="300px" class="is-required">
  ...
  export default {
		data(){
			return{
				...
				member_:'',
				placeholder:'请输入专家意见',
				pretrialExpertOpinion:[],
				projectId:JSON.parse(sessionStorage.getItem("rowPretrialInfo")).processVariables.projectId,
				rotation:JSON.parse(sessionStorage.getItem("rowPretrialInfo")).processVariables.budgetRotation,
			};
		},
		methods: {
			clickPic(id){
			...
			*/
			...
			/* 
			 上传意见附件
			 */
			handleRemoveComment(res, item, uploadName,file){
				if(res.meta.code == 0) {
			...
			*/
			reject(value){
				...
				this.axios.post(`${this.common.basePath}/activiti/complete`,formData).then( (response) => {
					if(response.data.meta.code === 0){
						// this.$message.success(response.data.meta.message, 20);
						this.$message.success('请等待申报单位上传预审改进方案后提交...', 20);
